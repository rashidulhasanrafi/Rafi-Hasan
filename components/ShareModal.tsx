...
import { Transaction, DashboardStats, TRANSLATIONS, Language, convertAmount, TransactionType } from '../types';
...
export const ShareModal: React.FC<Props> = ({
  ...
  const getExpenseBreakdown = () => {
    ...
    transactions
        ...
        .forEach(t => {
            ...
            categoryMap.set(t.category, currentVal + convertedVal);
        });
    ...
    const title = language === 'bn' ? 'খরচের বিবরণ:' : 'Expense Breakdown:';
  ...
  const getSummaryText = () => {
    ...
    return `
📊 *Hisab Report* (${profileName})
---------------------------
💰 ${t.totalBalance}: ${formatCurrency(stats.balance)}
📈 ${t.netIncome}: ${formatCurrency(stats.totalIncome)}
📉 ${t.netExpense}: ${formatCurrency(stats.totalExpense)}
---------------------------
${breakdown}
...
Generated by Hisab
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(14);
    doc.setTextColor(30);
    doc.text(t.title === 'Share & Export' ? 'Financial Summary' : 'আর্থিক সারাংশ', 30, 60);
    ...
    doc.setFontSize(14);
    doc.setTextColor(30);
    doc.text(language === 'en' ? "Top Expenses by Category" : "শীর্ষ ব্যয়ের খাতসমূহ", 20, 110);
    ...
    if (expenseData.length > 0) {
        ...
        doc.text(language === 'en' ? "No expense data available." : "কোনো ব্যয়ের তথ্য নেই।", 25, yPos);
    }
  ...
  const handleSharePDF = async () => {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      ...
      handleExportPDF();
      alert(language === 'bn' 
        ? 'আপনার ডিভাইসটি সরাসরি ফাইল শেয়ার সমর্থন করে না। পিডিএফটি ডাউনলোড করা হয়েছে। আপনি এটি ম্যানুয়ালি জিমেইল বা হোয়াটসঅ্যাপে সংযুক্ত করতে পারেন।'
